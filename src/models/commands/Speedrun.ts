...
export class WorldRecord implements ICommand {
  ...
  run = async () => {
    ...
    if (!game) return `Game ${gameName} not found`;
...
export class PersonalBest implements ICommand {
  ...
  run = async () => {
    ...
    if (!runner) return `Runner ${runnerName} not found`;
    const leaderboard = new Leaderboard(game);
    const fuzzyCategory = leaderboard.fuzzyCategory(query);
    const [{ item: category }] = fuzzyCategory;
    ...
    return `${runner.name} ${category.name} PB: ${personalBestTime} - #${personalBest.place} - ${daysAgo} days ago`;
  };
}
...
export class IndividualPersonalBestDiddyKongRacing implements ICommand {
  ...
  run = async () => {
    ...
    if(!run) return `Runner ${runner.name} has no pb for that track`;
    console.log(run)
    ...
    return `${runner.name} ${name} ${vehicle} PB: ${personalBestTime} - ${daysAgo} days ago`;
  }
}
  ...
  run = async () => {
    ...
    if (isSetting) {
      if (newUsername === undefined) {
        ...
        return `SpeedrunDotCom username restored to: ${channel}`;
      }
...
export class TimeTrialWorldRecordDiddyKongRacing implements ICommand {
  ...
  run = async () => {
    ...
    if (isShortcut === true) shortcut = "shortcut"
    ...
    return `${track} [${formalVehicle}][${laps}-lap]${shortcutPrint} WR: ${time} by ${runner} - ${daysAgo} days ago`;
  }
}
...
export class TimeTrialPersonalBestDiddyKongRacing implements ICommand {
  ...
  run = async () => {
    ...
    const shortcutPrint = shortcut === "shortcut" ? "[Shortcut]" : "";
    const daysAgo = datesDaysDifference(runDate)
    return `${run.username}'s ${track} [${placement}][${formalVehicle}][${laps}-lap]${shortcutPrint} PB: ${time} by ${runner} - ${daysAgo} days ago`;
  }
}
