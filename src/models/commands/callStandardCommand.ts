...
export class StandardCommand implements ICommand {
  ...
  buildCommands = () => {
    ...
    this.commandMap.set(CommandName.TRUST, new Trust(this.messageData))
  ...
  run = async () => {
    ...
    if (this.commandMap.has(chatterCommandUpper)) {
      // @ts-ignore
      return this.commandMap.get(chatterCommandUpper).run()
    }
